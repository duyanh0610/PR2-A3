...
/**
 * @overview process data about personal computer.
 * @attributes <pre>
 * model 			String 
 * year 			Integer 	int 
 * manufacturer  	String 
 ...
 * </pre>
 * @object	 <pre>
 * A typical PC is <m,y,ma,c> where model(m), year(y), manufacturer(ma), comps(c)
 * 
 * @abstract properties 
 * mutable (model)= T /\ optional(model)=F /\ length(model)=20 /\ 
 * mutable (year) = F /\ optional(year)= F /\ min(year) = 1940 /\
 * mutable(manufacturer) = F /\ optional(manufacturer)= F /\ length(manufacturer)=20 /\
 * mutable (comps) T /\ optional (comps) = F    
 *  </pre>
 * 
 * @author Duy Anh
 *
